...
# Import multilabel components if available
try:
    import multilabel_minimal
except ImportError:
    pass  # multilabel_minimal may not exist yet
...
def worker(proc_id, gpu_ranks, args, model):
    ...
    if args.dist_train:
        train_loader = dymdata.DataLoaders[args.target](
            args, rank, args.world_size, True
        )
    else:
    ...
    scheduler = WarmupLinearSchedule(
        optimizer, warmup_steps=args.total_steps * args.warmup, t_total=args.total_steps
    )
    ...
    tnr.TRAINERS[args.target](
        args, gpu_id, rank, train_loader, model, optimizer, scheduler
    )
...
def main():
    ...
    parser.add_argument(
        "--vocab_path",
        type=str,
        default="./vocab/bert_vocab.txt",
        help="Path of the vocabulary file.",
    )
    parser.add_argument(
        "--context_repo_path",
        type=str,
        default="./vocab/context_repo_init.txt",
        help="TXT of pre-collected context pieces.",
    )
    parser.add_argument(
        "--cellstr_repo_path",
        type=str,
        default="./vocab/cellstr_repo_init.txt",
        help="TXT of pre-collected context pieces.",
    )

    # model configuration options
    parser.add_argument(
        "--hidden_size", type=int, default=768, help="Size of the hidden states."
    )
    parser.add_argument(
        "--intermediate_size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Max magnitude of numeric values.",
    )
    parser.add_argument(
        "--precision_size",
        type=int,
        default=10,
        help="Max precision of numeric values.",
    )
    parser.add_argument(
        "--top_digit_size",
        type=int,
        default=10,
        help="Most significant digit from '0' to '9'.",
    )
    parser.add_argument(
        "--low_digit_size",
        type=int,
        default=10,
        help="Least significant digit from '0' to '9'.",
    )
    parser.add_argument(
        "--row_size", type=int, default=256, help="Max number of rows in table."
    )
    parser.add_argument(
        "--column_size", type=int, default=256, help="Max number of columns in table."
    )
    parser.add_argument(
        "--tree_depth",
        type=int,
        default=4,
        help="Maximum depth of top & left header tree.",
    )
    parser.add_argument(
        "--node_degree",
        type=str,
        default="32,32,64,256",
        help="Maximum number of children of each tree node.",
    )
    parser.add_argument(
        "--num_format_feature",
        type=int,
        default=11,
        help="Number of features of the format vector.",
    )
    parser.add_argument(
        "--attention_distance",
        type=int,
        default=8,
        help="Maximum distance for attention visibility.",
    )
    parser.add_argument(
        "--attention_step",
        type=int,
        default=0,
        help="Step size of attention distance to add for each layer.",
    )
    parser.add_argument(
        "--num_attention_heads",
        type=int,
        default=12,
        help="Number of the attention heads.",
    )
    parser.add_argument(
        "--num_encoder_layers",
        type=int,
        default=12,
        help="Number of the encoding layers.",
    )
    parser.add_argument(
        "--num_tcr_type", type=int, default=2, help="Number of table-context classes."
    )
    parser.add_argument(
        "--hidden_dropout_prob",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default="tuta",
        choices=["tuta", "tuta_explicit", "base", "multilabel"],
        help="Model variants.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=256,
        help="Maximum length of the table sequence.",
    )
    parser.add_argument(
        "--max_cell_num",
        type=int,
        default=256,
        help="Maximum cell number used in data loaders.",
    )
    parser.add_argument(
        "--max_cell_length",
        type=int,
        default=64,
        help="Maximum number of tokens in one cell string.",
    )
    parser.add_argument(
        "--max_disturb_num",
        type=int,
        default=20,
        help="Maximum number of cells to be disturbed per table.",
    )
    parser.add_argument(
        "--disturb_prob",
        type=float,
        default=0.15,
        help="Probability to be disturbed per cell.",
    )
    parser.add_argument(
        "--add_separate",
        type=bool,
        default=True,
        help="Whether to add [SEP] as aggregate cell representation.",
    )
    parser.add_argument(
        "--text_threshold",
        type=float,
        default=0.5,
        help="Probability threshold to sample text in data region.",
    )
    parser.add_argument(
        "--value_threshold",
        type=float,
        default=0.1,
        help="Prob to sample value in data region.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.3,
        help="Proportion of masked cells doing whole-cell-masking.",
    )
    parser.add_argument(
    ...
    # training options
    parser.add_argument(
        "--batch_size", type=int, default=12, help="Size of the input batch."
    )
    parser.add_argument(
        "--total_steps", type=int, default=1000000, help="Total training steps."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help="Specific steps to run validation (for multilabel).",
    )
    parser.add_argument(
        "--buffer_size",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50000,
        help="Mininum chunk size from a random data set.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="../dataset.pt",
        help="Paths of the preprocessed dataset.",
    )
    parser.add_argument(
        "--val_dataset_paths",
        type=str,
        default=None,
        help="Paths of the validation dataset (for multilabel).",
    )
    parser.add_argument(
        "--pretrained_model_path",
        type=str,
        default=None,
        help="Path of the pretrained bert/ts model.",
    )
    parser.add_argument(
        "--load_type", type=str, default="tuta", choices=["tuta", "bert", None]
    )
    parser.add_argument(
        "--output_model_path",
        type=str,
