...
class Backbone(nn.Module):
    def __init__(self, config):
        ...
        self.attn_methods = {"max": self.pos2attn_max, "add": self.pos2attn_add}
    ...
    def unzip_tree_position(self, zipped_position):
        ...
        entire_position = torch.zeros(batch_size, seq_len, self.total_node + 1).to(
            zipped_position.device
        )
        # Clamp positions to avoid negative indices (map -1 to total_node for not-applicable)
        zipped_position_clamped = torch.where(
            zipped_position < 0,
            torch.tensor(self.total_node, device=zipped_position.device),
            zipped_position,
        )
        entire_position = entire_position.scatter_(
            -1, zipped_position_clamped, 1.0
        ).long()
    ...
    def pos2attn(self, position):  # entire position
        ...
        vector_matrix = position.unsqueeze(2).repeat(
            1, 1, position.size()[1], 1
        )  # [batch, seq_len, seq_len, total_node]
    ...
    def create_post_mask(self, attn_dist, indicator, padding_dist=100):
        ...
        """
        cls_matrix = (
            (indicator == -1).long().unsqueeze(-1).repeat(1, 1, attn_dist.size(1))
        )
        cls_matrix = torch.max(cls_matrix, cls_matrix.transpose(-1, -2))
        cls_matrix = -(cls_matrix * attn_dist)
        pad_matrix = (
            (indicator == 0).long().unsqueeze(-1).repeat(1, 1, attn_dist.size(1))
        )
...
class BbForBase(Backbone):
    ...
    def forward(
        ...
        embedded_states = self.embeddings(
            token_id, num_mag, num_pre, num_top, num_low, token_order, format_vec
        )
...
BACKBONES = {"tuta": BbForTuta, "base": BbForBase, "tuta_explicit": BbForTutaExplicit}
