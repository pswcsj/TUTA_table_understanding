...
class Reader(object):
    ...
    def init_position_matrix(self, root, row_number, column_number, is_top):
        ...
        init_matrix = [
            [([-1] * self.tree_depth) for _ in range(column_number)]
        ...
        if is_top and (
            column_number <= self.node_degree[-1]
        ):  # equivalent of column position embedding
            for irow in range(row_number):
                for icol in range(column_number):
                    init_matrix[irow][icol][-1] = icol
        if (not is_top) and (
            row_number <= self.node_degree[-1]
        ):  # equivalent of row position embedding
            for irow in range(row_number):
                for icol in range(column_number):
    ...
    def get_merge_region(self, cell_row, cell_column, merged_regions):
        ...
        for merged_region in merged_regions:
            first_row, first_column = (
                merged_region["FirstRow"],
                merged_region["FirstColumn"],
            )
            last_row, last_column = (
                merged_region["LastRow"],
                merged_region["LastColumn"],
            )
            if (first_row <= cell_row <= last_row) and (
                first_column <= cell_column <= last_column
            ):
    ...
    def dfs_zipping(
        ...
        if children_num > 0:  # non-leaf
            if (level >= self.tree_depth) or (
                children_num > self.node_degree[level]
            ...
            for ichild, child_node in enumerate(children):
                ...
                child_position = (
                    node_position[:level] + [ichild] + node_position[level + 1 :]
                )
    ...
    def read_header(self, root, merged_regions, row_number, column_number, is_top):
        ...
        position_matrix = self.init_position_matrix(
            root, row_number, column_number, is_top
        )
...
class SheetReader(Reader):
    def info_from_matrix(
        self, cell_matrix, merged_regions, formula_value_augmentation=False
    ):
        """
        ...
        for cell_row in cell_matrix:
            ...
            for cell in cell_row:
                ...
                format_cell.append(
                    int((cell["BC"][0] == "#") and (cell["BC"].endswith("ffffff")))
                )
                format_cell.append(
                    int((cell["FC"][0] == "#") and (cell["FC"].endswith("000000")))
                )
    ...
    def result_from_file(self, json_path):
        ...
        if (row_number > self.row_size) or (column_number > self.column_size):
            print(
                "Fail for extreme sizes: {} rows, {} columns ".format(
        ...
        try:
            top_position_list = self.read_header(
                table["TopTreeRoot"], merged_regions, row_number, column_number, True
            )
            if top_position_list is None:
                print("Fail to generate top-tree position list")
                top_position_list = self.read_header(
                    None, merged_regions, row_number, column_number, True
                )
            left_position_list = self.read_header(
                table["LeftTreeRoot"], merged_regions, row_number, column_number, False
            )
            if left_position_list is None:
                print("Fail to generate the left-tree position list")
                left_position_list = self.read_header(
                    None, merged_regions, row_number, column_number, False
                )
        except:
        ...
        string_matrix, format_matrix = self.info_from_matrix(
            cell_matrix, merged_regions
        )
        header_rows, header_columns = (
            table["TopHeaderRowsNumber"],
            table["LeftHeaderColumnsNumber"],
        )
        return (
            string_matrix,
            (top_position_list, left_position_list),
            (header_rows, header_columns),
            format_matrix,
        )
